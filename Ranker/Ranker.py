...
class Ranker:
    ...
    #     docIDList = []
    #     for docNo in docNoList:
    #         docIDList.append(self.dictionary_document_info[docNo][0])
    #     return docIDList



    def getScore(self, docID:str, docDF:int, positionList:list, termDF:int) -> int:
        BM25Score = self.getBM25Score(docID=docID, docDF=docDF, termDF=termDF)
        # TODO - calculate the score in more ways
        return BM25Score



    def getBM25Score(self, docID:str, docDF:int, termDF:int):
        """
        BM25:
        ...
        log = math.log10((self.config.totalNumberOfFiles + 1) / termDF)
