...
class Parse:
    ...
    def parseDoc(self, documentAsString):
        ...
        try:
            ...
            countryLine = re.findall(r"<F P=101>(.+?)</F>", documentAsString[:int(len(documentAsString)/2)])
            # cityLine = documentAsString[documentAsString.find('<F P=104>') + len('<F P=104>'):documentAsString.find('</F>')]
            cityLine = re.findall(r"<F P=104>(.+?)</F>", documentAsString[:int(len(documentAsString)/2)])
            # languageLine = documentAsString[documentAsString.find('<F P=105>') + len('<F P=105>'):documentAsString.find('</F>')]
            languageLine = re.findall(r"<F P=105>(.+?)</F>", documentAsString[:int(len(documentAsString)/2)])
...
<TEXT>
Language: <F P=105> Chinese sadfa </F>
Article Type:BFN 
